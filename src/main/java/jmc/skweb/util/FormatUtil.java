...
//		String options = "<tr><td>122</td><td>La Filomena</td><td>0000090/123432</td><td>10/01/2012</td><td>1</td><td>1232.00</td><td>123.00</td><td>123.89</td><td>10/08/2012</td></tr></table>";
	...
	public SendMedicamentosDocument makeSendMedicamentosCompraDocument(TrazabiFarma tf, String usr, String pass)
	throws Exception {
		...
		m.setGlnOrigen(tf.getGlnorigenIng().trim());
		m.setGlnDestino(tf.getGlndestinoIng().trim()
				
		
		);
		m.setCuitOrigen(tf.getCuitOrigenIng());
	...
	public SendMedicamentosDocument makeSendMedicamentosEgresoDocument(
			TrazabiFarma tf,  String usr, String pass,
			DatosTrazaWS datos) {
		SendMedicamentosDocument req = null;
		try{
		req = SendMedicamentosDocument.Factory.newInstance();
		...
		m = makeDatosComunes(m,tf);
		m.setIdEvento(tf.getCodEventoSal().toString());
		m.setGlnOrigen(tf.getGlnorigenSal().trim());
		try{
			m.setGlnDestino(tf.getGlndestinoSal().trim());
		}catch(Exception e){}
		m.setCuitOrigen(tf.getCuitOrigenSal());
	...
	public static Object getListHTMLTrazabiFarma(
			List<TrazabiFarma> trazabiFarmaList) {

		String data = "";
		int sizeList= trazabiFarmaList.size();
		if (sizeList > 10)
			sizeList = 10;
		String header = " <select onclick=\"callTrazaSelectList();\" name=\"drop1\" id=\"idAutoCompleteManual\" size=\""+sizeList+"\" class = \"AutoComplete\">";
		String options = "";
		String footer = " </select> ";
		for(TrazabiFarma trazabiFarma: trazabiFarmaList){
			String option = "<option value=\""+trazabiFarma.getNr()+"\">" + trazabiFarma.getDescripC()+"</option>";
			options = options + option;
		}		
		data = header + options + footer;
