...
public class UsersAction extends ActionSupport  {
  ...
  /**
	...
	/**
	 * Muestra Todos los empresas de la empresa en que esta logeado el usuario  
	 * 
	 */
	public String getEmpresas() throws Exception {	        						
		...
		List<EmpresaWeb> empresaList = empresaManager.getEmpresas(); 
		
		request.setAttribute("empresaList", empresaList);
	...
	/**
	 * Prepara la edicion de un empresa  
	 * 
	 */
	...
	public String preparedEditEmpresa() throws Exception {	        						
		...
		empresaWeb = empresaManager.getEmpresaByPK(empresaWeb.getIdEmpresaWeb());
	...
	/**
	 * Guarda la Empresa save or update depende cada caso
	 * @return String
	 */
	public String saveEmpresa(){		
		...
		String leyendaSaveUsuario = "";
		if (adminEmpresa.equals("new")) {							
			
			try {
	        	//Verifico que el gln de la empresa no este dado de alta
				if (empresaManager.existeGln(empresaWeb.getGln())){
					leyendaSaveUsuario = "El GLN " + empresaWeb.getGln() + " ya existe en la base de datos.";
					sos.print(leyendaSaveUsuario);
					return null;
				}
				empresaManager.saveEmpresaWeb(empresaWeb);
				leyendaSaveUsuario = "OK";
				sos.print(leyendaSaveUsuario);
				return null;
			...
			//Edito el EmpresaWeb
			try {
				empresaManager.updateEmpresa(empresaWeb);
				leyendaSaveUsuario = "OK";
				sos.print(leyendaSaveUsuario);
				return null;
				...
				e.printStackTrace();
				leyendaSaveUsuario = "No se pudo editar la empresa por favor comuniquese con el administrador";
				try {
					sos.print(leyendaSaveUsuario);
				} catch (IOException e1) {
					e1.printStackTrace();
