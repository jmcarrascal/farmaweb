<%@ taglib prefix="s" uri="/struts-tags" %>

<%@ taglib uri="/WEB-INF/tlds/extremecomponents.tld" prefix="ec" %>
 
<link href="<%=request.getContextPath()%>/css/hints.css" rel="stylesheet" type="text/css" />
<link href="<%=request.getContextPath()%>/css/styles.css" rel="stylesheet" type="text/css" />






<div class="main">
<div align="right"><a href="preparedListRemitosPendientes"> <img
	src="<%=request.getContextPath()%>/images/general/back.png" width="50px" alt="Volver" title="Volver"
	 border="0"></img> </a> <a href="<%=request.getContextPath()%>/home"> <img
	src="<%=request.getContextPath()%>/images/general/homev1.png" width="50px" alt="Inicio" title="Inicio"
	border="0"></img> </a>
	</div>
			
		   	<fieldset class="defaultFieldset">
    			<legend><b>Remitos Enviados</b></legend>    			
	 			<table>					
				<tr>
					<td align="left">
						<s:label id="idRazonSocial" name="nombre" cssClass="entidadGrande"/>
					</td>										
				</tr>				

				</table>
					<div class = "separator"></div>
					<ec:table					
						tableId="notReg" 
				    	items="transacList"
						var = "transac"
						locale="es_ES"
						action="preparedListRemitosEnviados"
						imagePath="images/table/*.gif"
						width="100%"
						rowsDisplayed="100" >						
						<ec:exportPdf view="pdf" headerColor="white" headerTitle='<%=String.valueOf((request.getAttribute("grillaResult")))%>' fileName="soloImpagos.pdf" tooltip="Exportar PDF" />
						<ec:exportXls view="xls" text='<%=String.valueOf((request.getAttribute("grillaResult")))%>' fileName="soloImpagos.xls" tooltip="Exportar Excel"/>
						<ec:row>
								<ec:column width="50px" title=" " filterable="false" sortable="false" alias="read" viewsAllowed="html">
									<a href="#">
	          							<img src="images/general/download.png" alt="Descargar Remito" title="Descargar Remito" onclick="downloadRemito('${transac.transacNr}')" border="0" "></img>
	          							<img src="images/general/view.png" alt="Medicamentos" title="Medicamentos" onclick="viewMedicamentos('${transac.transacNr}')" border="0" ></img>
									</a>
								</ec:column>
								<ec:column width="150px" property="transacNr" title="TransacNr SKSoft"  >
									<span style="white-space:nowrap;" >${transac.transacNr}</span>
								</ec:column>
								<ec:column width="100px" property="fecha" title="Fecha" viewsAllowed="html">
									<span style="white-space:nowrap;" >${transac.formattedDate}</span>
								</ec:column>
								<ec:column width="100px" property="formattedDate" title="Fecha" viewsDenied="html"/>

								<ec:column width="100px" property="tipoComprob.descripcion" title="TipoComprob">
									<span style="white-space:nowrap;" >${transac.tipoComprob.descripcion}</span>
								</ec:column>
								<ec:column width="20px" property="letra" title="Letra">
									<span style="white-space:nowrap;" >${transac.letra}</span>
								</ec:column>								
								<ec:column width="auto" property="nrComprob" title="NrComprob">
									<span style="white-space:nowrap;" >${transac.nrComprob}</span>
								</ec:column>
																
								
						</ec:row>
					</ec:table>				
			</fieldset>					
</div>		


<s:form action="downloadRemito" id="downloadRemitoForm" target="_blank">
	<s:hidden key="transac.transacNr" name="transac.transacNr" id="idTransacNrHidden"/>	
</s:form>


<s:form action="viewTrazaEnviados" id="showMedicamentosForm">
	<s:hidden key="gente.genteNr" />
	<s:hidden key="transac.transacNr" id="idTransacNrViewHidden"/>
</s:form>

<script type="text/javascript">
function downloadRemito(transacNr){	
	$("#idTransacNrHidden").val(transacNr);
	document.getElementById("downloadRemitoForm").submit();
}

function viewMedicamentos(transacNr){	
	$("#idTransacNrViewHidden").val(transacNr);
	document.getElementById("showMedicamentosForm").submit();
}


</script>