...
<div align="right"><a href="preparedSendRemitos"> <img
	...
	<s:form id="addTrazabiFarmaForm" action="addTrazabiFarma" theme="simple" cssClass="inline" method="post" >		
			<s:hidden id = "trazabiFarmaHiddenId" key = "nrStr"/>
			<fieldset class="defaultFieldset">
				<legend><b>Selección de Medicación ingresada</b></legend>
				<table>
				<tr>
					<td><label for="articuloAuto1">Ingrese una porción de la Serie:</label></td>
					<td><s:textfield id="txtSerieId" name="trazabiFarma.serieGtin"
						size="20" maxlength="20" cssErrorClass="error"
						cssClass="ui-widget-content ui-corner-all" /></td>
					<td style="display: none" id="idLoadSerieGtin"><img alt=""
						src="images/general/loading1.gif"></td>
					<td style="display: inline" id="idBuscarPorSerieImage"><img alt=""
			...
			<table>		
			<tr>			
				<td><s:label id="descripArtID"  cssClass="entidadMedia"/></td>
			</tr>
			</table>
			<br>
			<table align="center">
			...
			<div style="display: inline;"  > 
				<img src="<%=request.getContextPath()%>/images/general/icon_agregar_caja.png" alt="Agregar Caja" title="Agregar Caja" onclick="addTrazabiFarma();"></img>
			</div>
			</td>
			</tr>
				</table>
...
<div class="main">			
			...
			<fieldset class="defaultFieldset">
				...
				<s:form id="enviarPorRemitoForm" action="enviarPorRemitoUnid" theme="simple" cssClass="inline" method="post" >
				<p id="validateTips"></p>				
				...
				<s:hidden key="transac.transacNr" name="transac.transacNr"/>
					...
					<table>
						<tr>
							...
							<td><s:textarea id="nroAfiliadoID" name="datosTrazaWS.obs" cols="90" rows = "3" maxlength="200" cssErrorClass="error" cssClass="ui-widget-content ui-corner-all"/></td>
						</tr>
			    	</table>				
...
$(document).ready(function() {
	...
	$("#idBuscarPorSerieImage").click(function() {
		$("div.ui").css({position:'absolute', left: ($("#txtSerieId").position().left), top: ($("#txtSerieId").position().top + 20) });			
		callTrazabiFarmaList();	
	});
...
function callTrazabiFarmaList() {
	nombreTrazabiFarma = $("#txtSerieId").val();				
	$("#idLoadSerieGtin").show();	
	$.ajax({
		type : "POST",
		url : "getTrazabiFarmaPorSerieList",
		data : {
			"q" : nombreTrazabiFarma
		},
		success : function(msg) {
			$("#listAutoComplete").html(msg);
			$("#listAutoComplete").show();
			$("#idLoadSerieGtin").hide();						
		}
	});	
}
...
function callTrazaSelectList() {
	$("#descripArtID").text($("#idAutoCompleteManual option:selected").html());
	$("#trazabiFarmaHiddenId").val($("#idAutoCompleteManual").val());	
	$("#txtSerieId").val('');
	$("#listAutoComplete").hide();				
}
...
function addTrazabiFarma(){
		if ($("#trazabiFarmaHiddenId").val() != ''){
			$("#addTrazabiFarmaForm").submit();
		}else{
